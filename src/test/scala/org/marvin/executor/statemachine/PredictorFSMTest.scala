...
class PredictorFSMTest extends TestKit(
  ...
  override def beforeAll(): Unit = {
    GenericHttpAPI.metadata = MetadataMock.simpleMockedMetadata().copy(reloadStateTimeout = Some(500))
    super.beforeAll()
  }
  ...
  "engine finite state machine" should {
    ...
    "start with Unavailable" in {
      val probe = TestProbe()
      val fsm = TestFSMRef[State, Data, PredictorFSM](new PredictorFSM(probe.ref))
